...
export default {
  ...
  methods: {
    getAllOrders () {
      // 建立promise陣列
      const promises = []
      axios.get(`${host}/api/${path}/admin/orders`)
        .then((res) => {
          console.log(res)
          // 先抓第一頁訂單資料
          ...
          for (let i = 2; i <= pages; i++) {
            ...
            const promise = axios.get(`${host}/api/${path}/admin/orders?page=${i}`)
            promise.then((res) => {
    ...
    getOrders (page = 1) {
      axios
        .get(`${host}/api/${path}/admin/orders?page=${page}`)
        .then((res) => {
    ...
    deleteOrder (id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.delete(`${host}/api/${path}/admin/order/${id}`)
            .then((res) => {
              Swal.fire({
                position: 'center',
                icon: 'success',
...
<template>
  ...
  <div>
    <div class="container">
          ...
          <div v-for="order in orders" :key="order.id" class="card mb-3">
            <div class="row g-0 position-relative">
              <div class="col-5 p-3">
                ...
                <h6 class="d-flex justify-content-between text-primary">
                  <span>姓名 :</span> <span class="text-dark">{{ order.user.name }}</span>
                </h6>
                <h6 class="d-flex justify-content-between text-primary">
                  <span>連絡電話 :</span> <span class="text-dark">{{ order.user.tel }}</span>
                </h6>
              </div>
              <div class="col-5 p-3">
                <h6 class="d-flex justify-content-between text-primary">
                  <span>Email :</span> <span class="text-dark">{{ order.user.email }}</span>
                </h6>
                <h6 class="d-flex justify-content-between text-primary">
                  <span>收件地址 :</span> <span class="text-dark">{{ order.user.address }}</span>
                </h6>
                <h6 class="d-flex justify-content-between text-primary">
                  <span>備註 :</span> <span class="text-dark">{{ order.message === undefined ?
                    '無特別備註' : order.message }}</span>
                </h6>
                <div class="row">
                  <div class="col-6">
                    <h6 class="d-flex justify-content-between text-primary">
                      <span>訂單金額 :</span> <span class="text-dark">{{ order.total }}</span>
                    </h6>
                  </div>
                  <div class="col-6">
                    <h6 class="d-flex justify-content-between text-primary"><span>訂單狀態 :</span>
      ...
      <pagination v-if="checkAll" :pages="pagination" @emit-pages="getOrders"></pagination>
